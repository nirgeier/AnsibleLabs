...
* This lab is based upon the [previous lab](../000-setup/README.md) and its `docker-compose`.
...
- How to create and configure `ansible.cfg`
- How to set up an Ansible `inventory` file
- How to configure `ssh.config` for Ansible
- How to verify Ansible configuration and connectivity

---

## Prerequisites

- Complete the [previous lab](../000-setup/README.md#usage) in order to have the `Ansible` controller and the `Linux` servers up and running.
...
- As this is the main configuration file for our demo application, it is the content of `ansible.cfg` which we will use in this lab.
  ...
  ```ini
  # File location: /home/ansible/ansible.cfg
...
- In this demo we will use our own `ssh.config` configuration file.
  ...
  ```ini
  # File location: /home/ansible/.ssh/config
...
- The inventory configuration we will use for the labs:
  ...
  ```ini
  # File location: $LABS/000-setup/inventory.yml
...
- Place the files under the shared folder or simply execute the script [/Labs/000-setup/02-init-ansible.sh](../000-setup/02-init-ansible.sh)
- Verify that the controller can execute ansible [/Labs/000-setup/01-init-servers.sh](../000-setup/01-init-servers.sh)
...
- `ansible --version` confirms the installed version and active configuration file
- The `ansible.cfg` file sets defaults: inventory path, remote user, private key, and SSH options
- The `ping` module tests SSH connectivity and Python availability on managed hosts - no packages needed on targets
- `ansible-inventory --list` validates the inventory and shows all host variables in JSON format
- A successful `pong` response confirms Ansible can authenticate, connect, and execute modules on the target host
